...
class DownlinkAPIInputAdapter(BaseInputAdapter):
    """
    Adapter for the Downlink Verification Service REST API.
    Reads connection details from environment variables if not passed explicitly:
      - DOWNLINK_ACCESS_API_URL
      - DOWNLINK_ACCESS_API_KEY
      - DOWNLINK_ACCESS_API_KEY_HEADER  
    """
    ...
    def __init__(
        ...
        self.api_url = (
            api_url
            or os.environ.get("DOWNLINK_ACCESS_API_URL")
            or os.environ.get("P1_ACCESS_API_URL")
        )

        self.api_key = (
            api_key
            or os.environ.get("DOWNLINK_ACCESS_API_KEY")
            or os.environ.get("P1_ACCESS_API_KEY")
        )

        self.header_name = (
            header_name
            or os.environ.get("DOWNLINK_ACCESS_API_KEY_HEADER")
    ...
    def fetch_records(self) -> List[AccessLogRecord]:
        """
        Fetches verified access telemetry records from the Downlink Verification Service API. using configured headers.
        Handles errors gracefully without crashing the Rule Engine.
        """
        if not self.is_configured():
            logger.info("Downlink API mode is not fully configured (missing P1_ACCESS_API_URL or P1_ACCESS_API_KEY).")
            return []
...
Person1APIInputAdapter = DownlinkAPIInputAdapter
Person1APIAdapterPlaceholder = DownlinkAPIInputAdapter
